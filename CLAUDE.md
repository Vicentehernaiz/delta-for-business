...
```typescript
// Valid segment values — THREE plans (updated April 2026)
type SegmentId = 'business-traveler' | 'enterprise' | 'large-enterprise'

// Plan mapping:
// business-traveler  → Free self-serve (was: individual + smb)
// enterprise         → MSA, 50–500 travelers, $50K+ (was: mid-market)
// large-enterprise   → CSA, 500+ travelers, $300K+ (was: enterprise)

// How to read it in a page
export default function Page({ searchParams }: { searchParams: { for?: SegmentId } }) {
  const segment = searchParams.for ?? 'business-traveler' // default
  ...
}
```
...
> **⚠️ Updated April 2026 — New 3-plan structure (confirmed with Delta client)**
> Old 4-plan structure (individual / smb / mid-market / enterprise) replaced by 3-plan structure below.
...
export const segments: Record<string, Segment> = {
  'business-traveler': {
    ...
    description: 'For individual travelers and small teams. Free self-serve, dual SkyMiles earning.',
  ...
  enterprise: {
    ...
    description: 'For companies with 50–500 travelers spending $50K+. MSA with Corporate Priority.',
  ...
  'large-enterprise': {
    ...
    description: 'For companies with 500+ travelers spending $300K+. CSA with global coverage + Delta SYNC.',
