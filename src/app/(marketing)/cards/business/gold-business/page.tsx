...
export const metadata: Metadata = {
  ...
  description:
    'Delta Gold Business Amex: $150/yr (waived year 1), 40K miles bonus, 2x on Delta + shipping + advertising. Best entry business card.',
  alternates: { canonical: 'https://business.delta.com/cards/business/gold-business' },
}
    ...
    <main>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              <p style={{ fontSize: 'var(--type-scale-12)', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.6)', marginBottom: '8px' }}>
                Business card · $150/yr (first year free)
              </p>
      ...
      {card.welcomeBonus && (
        <section className="py-8 px-6" style={{ background: 'rgba(217,119,6,0.06)', borderBottom: '1px solid var(--color-neutral-10)' }}>
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p style={{ fontSize: 'var(--type-scale-14)', color: 'var(--color-neutral-600)' }}>After ${card.welcomeBonus.spendRequirement.toLocaleString()} in the first {card.welcomeBonus.months} months.</p>
          </div>
        </section>
      )}
      ...
      <section className="py-12 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { icon: 'ph-fill ph-package', title: '2x on shipping & US advertising', body: 'Double miles on US shipping purchases and US media buys — categories that personal cards don\'t bonus.' },
              { icon: 'ph-fill ph-users', title: 'Domestic companion certificate', body: 'Annual Main Cabin companion certificate for domestic routes.' },
              { icon: 'ph-fill ph-check-square', title: 'Free first checked bag', body: 'First bag free for you and travel companions on Delta-operated flights.' },
              { icon: 'ph-fill ph-trend-up', title: `MQD boost: ${card.mqdBoostRate}`, body: 'Every $25 in purchases earns $1 MQD toward individual Medallion status.' },
            ].map((b) => (
      ...
      <section className="py-12 px-6" style={{ background: 'var(--color-neutral-5)' }}>
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-4 items-center justify-between">
          <div>
            <p style={{ fontSize: 'var(--type-scale-16)', fontWeight: '700', color: 'var(--color-delta-blue-700)', marginBottom: '4px' }}>Need faster Medallion progress?</p>
            <p style={{ fontSize: 'var(--type-scale-14)', color: 'var(--color-neutral-600)' }}>Platinum Business adds a $2,500 MQD head start + 3x on Delta for $350/yr.</p>
          </div>
          <div className="flex gap-3 flex-wrap">
            <Link href="/cards/business/platinum-business" className="inline-flex items-center gap-2 font-semibold" style={{ height: '44px', padding: '0 20px', borderRadius: 'var(--radius-full)', background: 'var(--color-neutral-0)', color: 'var(--color-delta-blue-700)', fontSize: 'var(--type-scale-14)', border: '1px solid var(--color-neutral-10)' }}>
